...
(function (root, factory) {
  ...
  async function open(media, episodes, startIndex, directUrl, overrideCandidates) {
    ...
    // OS-native video player. Any fullscreen exit while the player is active
    // must take the user back.
    bindExitHandlers();
  ...
  function loadNative(url) {
    ...
    elements.video.volume = 1.0;
    elements.video.muted = false;
    // Force inline playback: without playsinline, WebKit-based TV browsers
    // launch the OS-native fullscreen player which swallows the remote BACK
    // key. Inline playback inside this full-screen overlay looks identical.
    try {
  ...
  // watching. WebKit-based TV browsers play <video> in their own fullscreen
  // player (or the embed did) and the BACK key is consumed by it. Close
  // UNCONDITIONALLY: at this moment document.webkitFullscreenElement may not
  // be cleared yet, so gating on it can wrongly skip the close.
